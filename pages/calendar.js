              ...
              return (
                ...
                >
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <span style={{fontSize:14,fontWeight:isToday?700:400,color:isToday?"#3b82f6":"#f1f5f9"}}>{day}</span>
                    {items.length > 0 && (
                      <span style={{background:hasOverdue?"#ef4444":"#3b82f6",color:"#fff",fontSize:10,fontWeight:600,padding:"2px 6px",borderRadius:10}}>{items.length}</span>
                    )}
                  </div>
                  {items.length > 0 && (
                    <div style={{fontSize:11,color:"#94a3b8",overflow:"hidden"}}>
                      {items.slice(0,2).map((item, i) => (
                        <div key={i} style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",color:item.status==="Complete"?"#64748b":hasOverdue?"#fca5a5":"#94a3b8"}}>
                          {item.description.substring(0,15)}{item.description.length>15?"...":""}
                        </div>
                      ))}
                      {items.length > 2 && <div style={{color:"#64748b"}}>+{items.length-2} more</div>}
                    </div>
                  )}
                </div>
              )
        ...
        <div>
          <div className="card" style={{position:"sticky",top:80}}>
            <div style={{fontSize:16,fontWeight:600,color:"#f1f5f9",marginBottom:16}}>
              {selectedDate ? `${monthNames[month]} ${selectedDate}, ${year}` : "Select a day to view items"}
            </div>

            {!selectedDate && (
              <div style={{color:"#64748b",fontSize:14}}>Click on a day in the calendar to see all action items due that day.</div>
            )}

            {selectedDate && selectedItems.length === 0 && (
              <div style={{color:"#64748b",fontSize:14}}>No action items due on this day.</div>
            )}
            ...
            {selectedItems.length > 0 && (
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {selectedItems.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{background:isOverdue?"rgba(239,68,68,0.1)":isComplete?"rgba(16,185,129,0.1)":"#1e293b",border:isOverdue?"1px solid #ef4444":isComplete?"1px solid #10b981":"1px solid #334155",borderRadius:8,padding:12}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                        <span style={{fontWeight:500,color:isComplete?"#64748b":"#f1f5f9",textDecoration:isComplete?"line-through":"none"}}>{item.description}</span>
                        <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:isComplete?"#10b981":isOverdue?"#ef4444":"#3b82f6",color:"#fff"}}>{item.status}</span>
                      </div>
                      {priority && <div style={{fontSize:12,color:"#4a90d9",marginBottom:4,cursor:"pointer"}} onClick={() => router.push("/priority/"+priority.id)}>📋 {priority.title}</div>}
                      {owner && <div style={{fontSize:12,color:"#64748b"}}>👤 {owner.full_name}</div>}
                      <div style={{fontSize:12,color:"#64748b",marginTop:4}}>Progress: {item.completion_percentage||0}%</div>
                      <button className="btn btn-secondary btn-sm" style={{marginTop:8,width:"100%"}} onClick={() => router.push("/priority/"+item.priority_id)}>View Priority</button>
                    </div>
                  )
                })}
              </div>
